...
const CLAIM_BOILERPLATE_PATTERNS = [
  ...
  /^review and refine/i,
  /^confidence is /i,
  /^reviewed against (?:rendered pages and )?the extracted text/i,
  /^visually reviewed image capture/i,
  /^use this note (?:for|as) (?:grounded )?qualitative/i,
  /^return to the raw source if exact chart precision matters/i,
  /^treat (?:figure )?ocr as a routing aid/i,
  ...
  /^extraction status:/i,
  /^extraction method:/i,
  /^both pdfs were reviewed through rendered-page ocr/i,
  /^metadata was corrected from first-pass guesses/i,
  /^rendered-page ocr review completed/i,
  /^the document is chart-heavy/i,
  /^the ocr and chart framing look usable/i,
  /^exact numeric reuse should/i,
  /^the note is text-led/i,
